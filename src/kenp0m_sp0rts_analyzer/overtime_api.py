...
class OvertimeAPIClient:
    ...
    async def login(self) -> bool:
        ...
        try:
            result = await self._post("Login.asmx/CustomerSignIn", {
        ...
        for item in result.get("Data", []):
            ...
            if item.get("NextEventDateTime"):
                try:
                    ...
                    match = re.search(r'/Date\((\d+)\)/', item["NextEventDateTime"])
                    if match:
    ...
    async def get_sport_offering(
        ...
        """
        result = await self._post("Offering.asmx/GetSportOffering", {
            "sportType": sport_type,
            "sportSubType": sport_subtype,
            "sportSubTypeID": sport_subtype_id,
            "contestType": None,
            "contestType2": None,
            "contestType3": None,
            "wagerType": "S",  # Straight bet
        })
    ...
    def _parse_game_line(self, gl: dict, sport: OvertimeSport) -> OvertimeGame | None:
        ...
        try:
            ...
            # Extract team names
            away_team = gl.get("Team1", "").strip()
            home_team = gl.get("Team2", "").strip()
            ...
            if gl.get("Spread2") is not None:
                ...
                spread_away_odds = gl.get("SpreadAdj1") or gl.get("OrigSpreadAdj1")
                spread_home_odds = gl.get("SpreadAdj2") or gl.get("OrigSpreadAdj2")
            ...
            return OvertimeGame(
                ...
                away_team=away_team,
                home_team=home_team,
                away_rot=gl.get("RotNum1", 0),
                home_rot=gl.get("RotNum2", 0),
                spread=spread,
...
def save_vegas_lines(
    ...
    if output_path is None:
        output_path = Path(__file__).parent.parent.parent.parent / "data" / "vegas_lines.json"
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        parser.add_argument("--no-extra", action="store_true", help="Exclude College Extra games")
        ...
        async with OvertimeAPIClient() as client:
            if args.sports:
                ...
                for s in sports:
                    ...
                    print(f"  {status} {s.sport_type} - {s.sport_subtype} (ID: {s.sport_subtype_id})")
                return
            ...
            if args.output or True:  # Always save
                ...
                saved_path = save_vegas_lines(game_dicts, target_date, output_path)
                print(f"\n📁 Saved to: {saved_path}")
